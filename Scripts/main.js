    ...
    reload: function() {
        ...
        for (var i = this.figures.length; i--;) {
            if (this.figures[i] instanceof Mario) {
                ...
                if (settings.lifes == -1) {
                    if (confirm("ไม่นะ! ฉันแพ้แล้ว!! ฉันทำเรื่องนี้คนเดียวไม่ไหว ฉันต้องฝากพวกคุณช่วยต่อแล้วล่ะ ไปที่เว็บไซต์ของพวกฉันแล้วมาช่วยโลกกันเถอะ!!!")) {
...
var Mario = Hero.extend({
    init: function(x, y, level) {
        this.standSprites = [
            ...
            [
                [{ x: 0, y: 162 }, { x: 481, y: 247 }],
                [{ x: 81, y: 243 }, { x: 561, y: 247 }]
            ]
        ];
        this.crouchSprites = [
            [{ x: 241, y: 0 }, { x: 161, y: 0 }],
